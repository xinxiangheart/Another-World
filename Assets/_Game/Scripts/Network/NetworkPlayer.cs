...
public class NetworkPlayer : NetworkBehaviour
{
    ...
    void TargetRemoteCardPick(NetworkConnectionToClient _, int selectionId, RemoteCardPickKind kind, int[] allowedBaseCosts, string excludePrefix)
    {
        ...
        System.Func<CardInstance, bool> f = ci =>
        {
            ...
            if (!RemoteKindOk(kind, td)) return false;
            if (allowedBaseCosts != null && allowedBaseCosts.Length > 0)
            {
                bool ok = false;
                for (int i = 0; i < allowedBaseCosts.Length; i++)
    ...
    /// 修复"AI 回手误进玩家手牌"。oldInstance 仍按在场定位 owner。
    /// knownOwner 可选：卡已退场（不在槽位）时由调用方提前捕获的 owner，避免扫描失败回退到 Local。</summary>
    public static void ReturnCardToOwner(CardData template, CardInstance oldInstance, NetworkPlayer knownOwner = null)
    {
        if (template == null || oldInstance == null) return;
        NetworkPlayer owner = knownOwner;
        if (owner == null)
        {
